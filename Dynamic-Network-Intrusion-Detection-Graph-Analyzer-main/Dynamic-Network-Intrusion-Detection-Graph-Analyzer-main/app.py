...
@st.cache_data
def load_and_process_data(csv_path="network_traffic_data.csv"):
    if not os.path.exists(csv_path):
        return None, None, None, None
    
    # Run the full intrusion detection pipeline to get all data
    final_df, alerts_df = run_intrusion_detection(csv_path=csv_path, output_path="threat_alerts.csv")
...
with tab3:
    ...
    with col_b:
        ...
        fig_pie = px.pie(level_counts, values='count', names='threat_level', 
                        color='threat_level',
                        color_map={'High': '#ff4b4b', 'Medium': '#ffa500', 'Low': '#00d4ff'},
