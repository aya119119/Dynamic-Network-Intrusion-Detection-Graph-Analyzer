...
def _build_reason(row: pd.Series) -> str:
    ...
    if row.get("degree_score", 0.0) >= 0.7:
        reasons.append("high network degree")
    if row.get("port_scan_score", 0.0) >= 0.7:
        reasons.append("port sweeping")
    if row.get("port_entropy", 0.0) >= 4.0:
        reasons.append("high port randomness")
    if row.get("connection_density", 0.0) >= 10.0:
        reasons.append("rapid connection bursts")
    ...
    if not reasons:
        return "Elevated risk profile (aggregated behavioral signals)"

    reason_text = " + ".join(reasons)
    
    # Advanced logic for specific threat types
    if "port sweeping" in reasons or "high port randomness" in reasons:
        return f"{reason_text} -> Probable Port Scan / Reconnaissance"
    if "rapid connection bursts" in reasons and "temporal spike" in reasons:
        return f"{reason_text} -> Potential DoS/DDoS Activity"
    if "ML anomaly detected" in reasons and len(reasons) > 2:
        return f"{reason_text} -> High Confidence Intrusion Attempt"

    return f"{reason_text} -> Suspicious Behavioral Pattern"
